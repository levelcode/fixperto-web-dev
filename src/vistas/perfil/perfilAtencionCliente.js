...
class PerfilAtencionCliente extends React.Component {
    ...
    componentDidMount() {
        ...
        var nm = this.state['user'][0]['name']
        this.setState({name_user : nm})
    ...
    getDatasCustomerSupport = () => {
		...
		.then(function (responseJson) {
            ...
            if (responseJson.success) {
                    ...
                    me.setState({ 
                        showAlert: true, 
                        textoAlert: "Ha ocurrido un problema, inténtelo nuevamente" 
                    });
                }
		})
		.catch((error) => {
    ...
    crearTicket = () => {
		if (this.state["type"] === 0 || this.state["description"] === "") {
            ...
            axios({
                ...
                data    : {
                    user        : id,
                    type_customer_support: this.state["type"],
    ...
    responderTicket = () => {
		if (this.state["response"] === "") { 
            ...
            axios({
                ...
                data    : {
                    id      : me.state["ticket"]["id"], 
                    response: me.state["response"]
                },
                headers : { Accept: 'application/json' }
            })
            .then(function (responseJson) {
    ...
    closedTicket = id => {
        ...
        .then(function (responseJson) {
            responseJson = responseJson['data']
            if (responseJson.success) {
                me.setState({description : ""})
                me.setState({ showAlert: true, textoAlert: "Se ha generado el ticket" });
                me.getDatasCustomerSupport();
			} else { 
	...
	render() {
		...
		const { show_add, user, status, types, tickets, description, type, ticket, response, name_user } = this.state;
                        ...
                        <div className="w3-modal-content w3-animate-top w3-card-2 w3-round-large" style={{ width: 50 + '%', marginTop : 0 }}>
                                ...
                                <div className="w3-container w3-center" >
                                    ...
                                    <div className="w3-margin-top w3-margin-bottom">
                                        ...
                                        <p style={{textAlign : "left"}}> <span className="text_blue">Responder Ticket: </span>
                                        </p>
                        ...
                        <table className=" w3-table-all w3-margin-bottom">
                            ...
                            {tickets.map((item, key) => (
                                <tr key={key} >
                                    ...
                                    onClick={(e) => {
										e.preventDefault();
										this.getDataCustomerSupport(item.id)
									}}
                                    >{item.type_customer_support}</td>
                                    <td
                                    onClick={(e) => {
										e.preventDefault();
										this.getDataCustomerSupport(item.id)
                                    ...
                                    >{item.date_registry}</td>
                                    <td><img src="../../../assets/iconos/eliminar.png" style={{width: 20, height : 20, alignItems: "center"}} onClick={(e) => {
										e.preventDefault();
