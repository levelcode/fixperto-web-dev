			...
			.then(function (responseJson) {
				...
				if (responseJson.success) {
					if (responseJson.validate) {
						...
						return me.setState({ showAlert: true, textoAlert: "Código incorrecto, inténtelo nuevamente"  });
					}
				}
			})
		...
		if (this.state['new_phone'] !== '') {
			if (this.validatePhone(this.state["new_phone"])) {
				...
				.then(function (responseJson) {
					...
					if (responseJson.success){ 
						me.setState({ showAlert: true, textoAlert: "El teléfono ha sido cambiado" }); 

						me.setState({showCahngePhone : false})
					}
					else { 
			...
			me.setState({ showAlert: true, textoAlert: "Ingresa un número de teléfono" }); 
		}
