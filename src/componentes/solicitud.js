	...
	const getRequest = () => {
		return axios({
			...
			.then(function (responseJson) {
				if (responseJson["data"]["success"]) { setRequest(responseJson["data"]["request"]); }
			...
			<div className="w3-modal w3-text-black" style={{ display: (props["show"]) ? "flex" : "none" }}>
				<div className="w3-modal-content w3-animate-top w3-round-large" style={{ width: 50 + '%' }}>
					<div className="w3-container w3-margin-top w3-margin-bottom">
						...
						{request["photos"] && request["photos"].length > 0 && <div className="w3-section w3-center">Fotos</div>}
							...
							{request["photos"] && request["photos"].length > 0 && request["photos"].map((photo, key) => (
								<div className="w3-col s6 m3 l2" key={key}>
									<div className="w3-card card_serv">
										<img src={"https://api.fixperto.com/uploads/requests/" + photo["img"]} className="img_serv" alt="Foto"></img>
									</div>
								</div>
							))}
						</div>
